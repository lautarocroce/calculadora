...
function App() {
  ...
  const handleDatosChange = (e) => {
    ...
    if (name === 'fechaElaboracion') {
      const fechaVencimiento = datos.fechaVencimiento;
      if (fechaVencimiento && new Date(value) > new Date(fechaVencimiento)) {
        alert('La fecha de elaboración no puede ser posterior a la fecha de vencimiento');
        e.target.value = '';
      } else {
        setDatos((prev) => ({ ...prev, [name]: value }));
      }
    } else if (name === 'fechaVencimiento') {
      const fechaElaboracion = datos.fechaElaboracion;
      const fechaVencimiento = value;
      if (fechaVencimiento < fechaElaboracion) {
        alert('La fecha de vencimiento no puede ser anterior que la fecha de elaboración');
        e.target.value = '';
      } else {
        setDatos((prev) => ({ ...prev, [name]: value }));
      }
  ...
  const validarIngredientes = () => {
    ...
    for (const ing of ingredientes) {
      if (ing.nombre) {
        ...
        const min = ingredienteDefinido.min * datos.pesoTotal / 100;
        const max = ingredienteDefinido.max * datos.pesoTotal / 100;
        if (ingredienteDefinido) {
          if (ing.usado < min || ing.usado > max) {
            return `El ingrediente "${ing.nombre}" debe estar entre ${min}% y ${max}%.`;
          }
          totalUsado += ing.usado;
        }
      }
    }
    if (totalUsado > datos.pesoTotal) return `El total del peso de los ingredientes no puede superar el peso total "${datos.pesoTotal}".`;
    return null;
  };
  ...
  const generarPDF = () => {
    ...
    const tablaIngredientes = ingredientes
      ...
      .map((ing) => [
        ing.nombre,
        `${ingredientesLista.find((i) => i.nombre === ing.nombre)?.min * datos.pesoTotal / 100 || ''}`,
        `${ingredientesLista.find((i) => i.nombre === ing.nombre)?.max * datos.pesoTotal / 100 || ''}`,
        `${ing.usado}`,
        calcularPorcentaje(ing.usado).toFixed(2),
      ]);

    doc.autoTable({
      startY: 90,
      head: [['Ingrediente', 'Min gr', 'Max gr', 'Usado gr', 'Porcentaje']],
                ...
                <tr>
                  ...
                  <td>
                    <input
                      ...
                      onChange={handleDatosChange}
                      className="form-control"
                      disabled={!datos.fechaElaboracion}
                    />
                  </td>
                </tr>
                <tr>
            ...
            <table className="table table-success table-striped text-center mx-auto">
              <thead>
                <tr>
                  ...
                  <th style={{ width: "15%" }}>Min gr</th>
                  <th style={{ width: "15%" }}>Max gr</th>
                  <th style={{ width: "15%" }}>Usado gr</th>
                ...
                {ingredientes.map((ingrediente, index) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td>
                        {ingrediente.usado
                          ? calcularPorcentaje(ingrediente.usado).toFixed(2)
                          : ""}
                      </td>
                ...
                {ingredientes
                  ...
                  .map((ingrediente, index) => (
                    <tr key={index}>
                      ...
                      <td>{ingrediente.usado} g</td>
                      <td>{calcularPorcentaje(ingrediente.usado).toFixed(2)} %</td>
                    </tr>
                  ))}
              </tbody>
            </table>
